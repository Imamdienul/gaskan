...
class Berlangganan_model extends CI_Model {
    ...
    $this->db->select('b.*, c.fullname, c.phone_customer, rc.jenis_layanan, rc.bandwidth, rc.alamat_pemasangan, rc.nama_lengkap');
    ...
    public function get_pppoe_status($username) {
    // Query untuk memeriksa apakah user aktif di tabel radacct
    $this->db->select('acctstoptime, acctstarttime');
    ...
    if ($query->num_rows() > 0) {
        ...
        if ($row->acctstoptime === NULL) {
            return 'online'; // User sedang online
        } else {
            return 'offline'; // User terakhir offline
        }
    } else {
        return 'never'; // User belum pernah login
    }
}

// Fungsi untuk mendapatkan status semua PPPoE user
public function get_all_pppoe_status() {
    $berlangganan = $this->get_all_berlangganan();
    $result = [];
    
    foreach ($berlangganan as $row) {
        $status = $this->get_pppoe_status($row->username);
        $result[$row->id_berlangganan] = [
